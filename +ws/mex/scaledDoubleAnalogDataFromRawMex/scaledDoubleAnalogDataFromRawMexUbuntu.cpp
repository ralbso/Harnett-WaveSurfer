...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])  {
    ...
    if (nCoefficients==0) {
        for (mwIndex j=0; j<nChannels; ++j) {
            const double scaledDatum = 0.0 / channelScales[j];
            double *targetStart = scaledData + j*nScans;
            double *targetEnd   = targetStart + nScans;
        ...
        for (mwIndex j=0; j<nChannels; ++j) {
            ...
            const double* c = scalingCoefficients + j*nCoefficients;
            const double scaledDatum = c[0] / thisChannelScale;
            ...
            for (double *target = targetStart; target < targetEnd; ++target) {
                *target = scaledDatum;
                ++source;  // CRITICAL: advance source even though we don't use it
            }
        }
    }
